...
def region_to_index():
    df = pd.read_excel("Data_v3_process.xlsx", sheet_name='hotel', usecols=[6],names=None,keep_default_na=False)
...
for col in range(1,6):
    df = pd.read_excel("Data_v3_process.xlsx", sheet_name='hotel', usecols=[col],names=None,keep_default_na=False)
    df_li = df.values.tolist()
    data = []
    result = []
    new = []
...
#read rating
df_rating = pd.read_excel("Data_v3_process.xlsx", sheet_name='hotel', usecols=[8],names=None,keep_default_na=False)
